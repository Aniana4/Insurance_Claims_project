...
Blind_Submodel_na <- traindf[is.na(traindf$Blind_Submodel),]
...
train_no_completos <- traindf[!complete.cases(traindf),]
train_completos <- traindf[complete.cases(traindf),]
class(traindf)

traindf_ostia <- traindf[traindf$Claim_Amount>0,]
dim(traindf_ostia)

train_no_completos_Ostia <- train_no_completos[train_no_completos$Claim_Amount>0,]

levels(traindf$Cat10)
levels(traindf$Cat11)
levels(traindf$OrdCat)

Cat10_na <- traindf[is.na(traindf$Cat10),] #3917
Cat11_na <- traindf[is.na(traindf$Cat11),] #31469 
Cat12_na <- traindf[is.na(traindf$Cat12),] #0
ordcat_na <- traindf[is.na(traindf$OrdCat),] #7546

summary(traindf$Cat11)
...
#Sustitución con los modelos NA por desconocido


library(gdata)
ver <- NAToUnknown(traindf$Blind_Submodel, unknown="Desconocido")
#isUnknown(x=traindf$Blind_Submodel)
summary(traindf)

prueba <- traindf[is.na(traindf$Blind_Submodel),]
head(traindf$Blind_Submodel,1000)
levels(traindf$Blind_Submodel)
```
